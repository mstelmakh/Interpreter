...
class Scanner:
    ...
    def error(self, message: str) -> None:
        self.is_error = True
        error = LexerError(self.stream.line, self.stream.column, message)
    ...
    def try_build_number(self) -> bool:
        ...
        self.token = self.create_token(TokenType.NUMBER, integer + fraction)
        return True
    ...
    def try_build_ident_or_keyword(self) -> bool:
        ...
        if value_str in KEYWORDS_MAP:
            ...
            self.token = self.create_token(TokenType.IDENTIFIER, value_str)
        return True
    ...
    def try_build_operator_or_comment(self) -> bool:
        ...
        if not (is_beginning_of_composite or is_beginning_of_comment):
            self.token = self.create_token(SINGLE_CHAR_MAP[self.current_char])
            self.current_char = self.stream.advance()
            return True
        ...
        if lexeme == COMMENT_CHAR:
            ...
            # For example if '=' and '==' are an operators, but we have '=-'
    ...
    def create_token(self, token_type: TokenType, value: any = None) -> None:
        return Token(token_type, value, self.stream.line)
